...
class SevenZipProgressCallback
...
public:
	...
	*/
	...
	void WaitUntilComplete() {while(!m_complete);} // TODO: Wait on mutex
private:
...
ZIPFile::ZIPFile(std::unique_ptr<BaseZipFile> baseZipFile)
	: m_baseZipFile(std::move(baseZipFile))
{}
...
{
	return m_baseZipFile->GetFileList(outFileList);
}

bool ZIPFile::ReadFile(const std::string &fileName,std::vector<uint8_t> &outData,std::string &outErr)
{
	return m_baseZipFile->ReadFile(fileName,outData,outErr);
}

bool ZIPFile::AddFile(const std::string &fileName,const void *data,uint64_t size,bool bOverwrite)
{
	return m_baseZipFile->AddFile(fileName,data,size,bOverwrite);
}

bool ZIPFile::AddFile(const std::string &fileName,const std::string &data,bool bOverwrite)
{
	return AddFile(fileName,data.data(),data.size(),bOverwrite);
}
#pragma optimize("",on)
